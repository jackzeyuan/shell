...
DEST_IP=$1
echo "\033[33m deploy to ip= $DEST_IP, project=obd-service  \033[0m"


cd $PROJECT_HOME
git pull --rebase

cd $PROJECT_HOME/obd-service
if [ $? -ne 0 ]
then
    echo "Error"
    exit 1
...
if [ $? -ne 0 ]
then
    echo "\033[31m error in mvn service \033[0m"
    exit 1
fi

ssh $DEST_IP "cp -r /data/www/service.obd.chewen.com /data/www/service.obd.chewen.com.`date +%Y%m%d%H%M%S`"
if [ $? -ne 0 ]
then
...
rsync -rvctz --progress --delete $PROJECT_HOME/obd-service/target/obd-service-1.0-SNAPSHOT/ $DEST_IP:/data/www/service.obd.chewen.com/
if [ $? -ne 0 ]
then
