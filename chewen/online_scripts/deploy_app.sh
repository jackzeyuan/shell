...
ssh $DEST_IP "cp -r /data/www/app.obd.chewen.com /data/www/app.obd.chewen.com.`date +%Y%m%d%H%M%S`"
if [ $? -ne 0 ]
then
...
rsync -rvctz --progress --delete $PROJECT_HOME/obd-app/target/obd-app-1.0-SNAPSHOT/ $DEST_IP:/data/www/app.obd.chewen.com/
if [ $? -ne 0 ]
then
