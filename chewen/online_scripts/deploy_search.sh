...
DEST_IP=$1
echo "\033[33m deploy to ip= $DEST_IP, project=obd-search  \033[0m"


cd $PROJECT_HOME
git pull --rebase

cd $PROJECT_HOME/obd-search
if [ $? -ne 0 ]
then
    echo "Error"
    exit 1
...
if [ $? -ne 0 ]
then
    echo "\033[31m error in mvn search \033[0m"
    exit 1
fi

ssh $DEST_IP "cp -r /data/www/search.obd.chewen.com /data/www/search.obd.chewen.com.`date +%Y%m%d%H%M%S`"
if [ $? -ne 0 ]
then
...
rsync -rvctz --progress --delete $PROJECT_HOME/obd-search/target/obd-search-1.0-SNAPSHOT/ $DEST_IP:/data/www/search.obd.chewen.com/
if [ $? -ne 0 ]
then
