...
ssh $DEST_IP "cp -r /data/www/shop/admin.shop.com /data/www/shop/admin.shop.com.`date +%Y%m%d%H%M%S`"
if [ $? -ne 0 ]
then
...
rsync -rvctz --progress --delete $PROJECT/shop-admin/target/shop-admin-1.0-SNAPSHOT/ $DEST_IP:/data/www/shop/admin.shop.com/
if [ $? -ne 0 ]
then
