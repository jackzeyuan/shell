...
cd $PROJECT/shop-gateway-server
if [ $? -ne 0 ]
then
...
ssh $DEST_IP "cp -r /data/www/shop/gateway.shop.com /data/www/shop/gateway.shop.com.`date +%Y%m%d%H%M%S`"
if [ $? -ne 0 ]
then
...
rsync -rvctz --progress --delete $PROJECT/shop-gateway-server/target/shop-gateway-server-1.0-SNAPSHOT/ $DEST_IP:/data/www/shop/gateway.shop.com/
if [ $? -ne 0 ]
then
